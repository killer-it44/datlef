...
exports.down = async function (db) {
    await db.runSql(`REVOKE ALL ON SCHEMA public FROM ${user}`)
    await db.runSql(`REVOKE ALL PRIVILEGES ON ALL TABLES IN SCHEMA public FROM ${user}`)
    await db.runSql(`REVOKE ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public FROM ${user}`)
    await db.runSql(`REVOKE ALL ON SCHEMA datlef FROM ${user}`)
    await db.runSql(`REVOKE ALL PRIVILEGES ON ALL TABLES IN SCHEMA datlef FROM ${user}`)
    await db.runSql(`REVOKE ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA datlef FROM ${user}`)
    await db.runSql(`DROP USER ${user}`)
    await db.runSql('DROP SCHEMA datlef CASCADE')
